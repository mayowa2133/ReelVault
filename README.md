...
For YouTube, ReelVault accepts common watch, Shorts, live, embed, `youtube.googleapis.com` `/v/...`, nocookie embed, clip, attribution, redirect, consent redirect, source Shorts, legacy `watch_popup`/`movie_popup`, root `?v=...`, semicolon-separated `;v=...` query URLs, legacy hash/fragment video URLs, YouTube Kids URL shapes, and yt-dlp-supported single-video wrapper hosts such as `deturl.com`, `pwnyoutube.com`, `hooktube.com`, `yourepeat.com`, `tube.majestyc.net`, `vid.plus`, `zwearz.com`, and `cleanvideosearch.com`. It normalizes these to a stable YouTube URL before download. Protocol-relative YouTube URLs are normalized to `https://...`. YouTube playlist and channel live-stream embeds such as `/embed/videoseries` and `/embed/live_stream` are ignored because they are not stable single-video URLs.
