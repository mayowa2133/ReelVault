...
| `YT_DLP_IMPERSONATE_CLIENT` | yt-dlp request impersonation target such as `chrome`, `chrome-120`, `safari`, or `any`. Defaults to `any` so anonymous requests can use curl-cffi/browser TLS impersonation. Set empty to disable. |
...
ReelVault uses yt-dlp anonymously by default for public YouTube, Instagram media, TikTok, and X/Twitter video URLs. Old `INSTAGRAM_COOKIES_*` values are ignored unless `ENABLE_AUTH_COOKIES=true`, which prevents stale cookies from breaking public no-auth downloads. The Docker build installs the pinned stable `yt-dlp[default,curl-cffi]` package, then upgrades to the official May 25, 2026 yt-dlp master snapshot by default so recent extractor/networking fixes are available before the next stable release. `YT_DLP_IMPERSONATE_CLIENT` defaults to `any`, using yt-dlp's curl-cffi/browser TLS impersonation support without account authentication.

For YouTube bot-check responses, ReelVault automatically tries a no-auth fallback chain. It retries with yt-dlp's mobile web player client while skipping initial YouTube webpage/config requests, then tries explicit Android/iOS clients, broader YouTube client probing, anonymous YouTube Visitor Data, Visitor Data retries, Safari web, embedded-player, TV, Android VR, web creator, web music, TV simplified, and downgraded TV clients while skipping the watch page and config requests where useful. Visitor Data retries also set `youtubetab:skip=webpage` so the generated anonymous visitor value is not replaced by a webpage request. If `YOUTUBE_PO_TOKEN` is set, it also tries configured PO Token strategies. `YOUTUBE_FETCH_POT_POLICY`, `YOUTUBE_INCLUDE_MISSING_POT_FORMATS`, and `YOUTUBE_USE_AD_PLAYBACK_CONTEXT` expose the remaining yt-dlp YouTube PO-token knobs for controlled experiments. These paths follow yt-dlp's documented no-cookie Visitor Data and PO Token hooks. They can work around some Cloud Run datacenter IP challenges without account cookies, but they are still not guaranteed for every URL.
...
For Instagram failures, ReelVault accepts `/reel/`, `/reels/`, username-prefixed media URLs, `/p/`, `/tv/`, `m.instagram.com` mobile media URLs, `instagram://media?id=...` app deep links, story item URLs, and matching share URLs including `/share/reels/...`, retries common URL/embed variants, resolves `instagram.com/share/...` redirects before retrying, then makes a final anonymous public-page pass that downloads direct `og:video` or embedded `video_url` media if Instagram exposes one in the public HTML. Instagram `/reels/audio/...` pages are ignored because they are audio pages, not single-video media URLs.

For YouTube, ReelVault accepts common watch, Shorts, live, embed, `youtube.googleapis.com` `/v/...`, nocookie embed, clip, attribution, redirect, consent redirect, source Shorts, legacy `watch_popup`/`movie_popup`, root `?v=...`, semicolon-separated `;v=...` query URLs, legacy hash/fragment video URLs, YouTube Kids URL shapes, yt-dlp-supported single-video wrapper hosts such as `deturl.com`, `pwnyoutube.com`, `hooktube.com`, `yourepeat.com`, `tube.majestyc.net`, `vid.plus`, `zwearz.com`, and `cleanvideosearch.com`, plus known Invidious single-video URL forms. It normalizes these to a stable YouTube URL before download. Protocol-relative YouTube URLs are normalized to `https://...`. YouTube playlist and channel live-stream embeds such as `/embed/videoseries` and `/embed/live_stream` are ignored because they are not stable single-video URLs.
