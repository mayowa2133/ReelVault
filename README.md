...
For YouTube, ReelVault accepts common watch, Shorts, live, embed, nocookie embed, clip, attribution, redirect, source Shorts, legacy `watch_popup`/`movie_popup`, root `?v=...`, and YouTube Kids URL shapes, then normalizes them to a stable single-video URL before download. Protocol-relative YouTube URLs are normalized to `https://...`. YouTube playlist and channel live-stream embeds such as `/embed/videoseries` and `/embed/live_stream` are ignored because they are not stable single-video URLs.
